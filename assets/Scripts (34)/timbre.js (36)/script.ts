...
//     "use strict";
    ...
    var _envtype = (typeof window !== "undefined") ? "browser" : "superpowers";
    var _envmobile = false; // for now
    var _f64mode = false;
    var _bpm = 120;
    ...
    var SoundSystem = (function() {
        ...
        $.rec = function() {
            ...
            if (this.deferred) {
                Sup.log("rec deferred is exists??");
                return dfd.reject().promise();
            }

            if (this.status !== FINISHED_STATE) {
                Sup.log("status is not none", this.status);
                return dfd.reject().promise();
            }
...
(function(T) {
    ...
    $.on = $.addListener = function(type, listener) {
        if (type === "mml") {
            type = "data";
            Sup.log("A 'mml' event listener was deprecated in ~v13.03.01. use 'data' event listener.");
        }
        this._.events.on(type, listener);
        return this;
    };
