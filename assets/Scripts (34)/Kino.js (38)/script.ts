...
ae.Conductor = function(bpm, timesig, transitionBeats, players, section, function_downbeat, function_upbeat, function_stop) {
    ...
    this.metroFunction = function(count) {
        ...
        if (conductor.transitionBeats.indexOf(beat) >= 0) {
            Sup.log(beat + " is a transition beat in " + conductor.transitionBeats.toString());
...
ae.Conductor.prototype.start = function() {
    ...
    if (this.all_loaded) {
        ...
        Sup.log("loops not yet all loaded; try again later~");
    }
}
    ...
    if (this.all_loaded) {
        ...
        for (var i=0; i<this.players.length; i++) {
            ...
            if (loop.init && loop.loop && loop.init.isLoaded && loop.loop.isLoaded) {
                // Sup.log("- loaded both " + loop.url_init + " and " + loop.url_loop);
                
                // Check tail
                for (var j=0; j<loop.tail.length; j++) {
                    if (!(loop.tail[j].audio && loop.tail[j].audio.isLoaded)) {
                        Sup.log("- haven't loaded " + loop.tail[j].url);
                        this.all_loaded = false;
                        return;
                    }
                    else {
                        // Sup.log("- loaded " + loop.tail[j].url);
                    }
                }
                // Sup.log("- finished checking init,loop, and tail");
            }
            else {
                Sup.log("- haven't loaded one of either " + loop.url_init + " or " + loop.url_loop);
                this.all_loaded = false;
                return;
            }
        }
    ...
    if (this.tailActivated) {
        ...
        for (var i=0; i<this.tail.length; i++) {
            ...
            if (tail.beats.indexOf(beat) >= 0) {
                tail.audio.play();
                tail.audio.bang();
                Sup.log("playing tail: " + tail.url + " on beat " + beat);
                this.activated = false;
                Sup.log("deactivated");
                return;
            }
        }
        Sup.log("invalid beat " + beat);
        ...
        this.loop.play();
        this.loop.bang();
        Sup.log("playing loop: " + this.url_loop);
    }
    else {
        this.init.play();
        this.init.bang();
        this.initPlayed = true;
        Sup.log("playing init: " + this.url_init);
    }
}
...
ae.to_audio = function(url) {
    ...
    return T("audio").loadthis(url, function() {
        Sup.log("Done loading " + url);
    }, function(e) {
        Sup.log("Failed to load " + url + ": " + e);
    });
}
